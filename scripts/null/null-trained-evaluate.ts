...
/**
 * Shared with `./null-trained-evaluate-graph-list.ts` (the extracted WP3
 * `--graph-list`/`--trained-dir` mode, a thermo-maintainability review
 * finding once this file crossed 1000 lines): both modes rescore the same
 * held-out seed range, tick count, hidden size, and default shard count, so
 * these are exported rather than each module hard-coding its own copy of
 * the same literals.
 */
export const DEFAULT_HELD_OUT_START = 30001;
export const DEFAULT_HELD_OUT_COUNT = 100;
export const DEFAULT_TICKS = 1800;
export const DEFAULT_SHARDS = 8;
...
export const DEFAULT_HIDDEN_SIZE = 16;
 ...
 */
export const findSingleSubdirectory = (parentDir: string, label: string): string => {
...
export const requireFile = (path: string, label: string): void => {
 ...
 */
export const readBundleD = (armBundlePath: string): number => {
...
export const assertMatchingD = (bundlePathsByGraphId: ReadonlyMap<string, string>): void => {
 ...
 */
export const reconcileCemConfig = (
...
// ---------------------------------------------------------------------------
// --graph-list dispatch (pathway-interventions WP3)
// ---------------------------------------------------------------------------

/**
 * `--graph-list`/`--trained-dir` mode (rescoring the WP3 intervention/control
 * graphs' trained runs) lives in `./null-trained-evaluate-graph-list.ts`,
 * extracted out of this file once its addition pushed this file to exactly
 * 1000 lines (a thermo-maintainability review finding, mirroring the same
 * extraction `null-evaluate.ts` underwent one commit earlier in this plan —
 * see `graph-list-index.ts`'s own doc comment). Dispatching on `--graph-list`'s
 * presence, rather than a separate `--mode` flag, is unambiguous (the two
 * modes' argument sets are otherwise disjoint enough -- `--rewired-trained-dir`
 * vs `--trained-dir`, etc. -- and each parser rejects the other's flags) and
 * matches `null-evaluate.ts`'s own `--graph-list`-selects-a-mode convention
 * (WP2).
 *
 ...
 * after this file has fully finished initializing, so the extracted
 * module's own imports of this file are always fully defined by the time it
 * loads.
 */
const dispatchMain = async (): Promise<void> => {
  if (process.argv.slice(2).includes('--graph-list')) {
    const { runInterventionMain } = await import('./null-trained-evaluate-graph-list');
    await runInterventionMain();
  } else {
    await main();
  }
};
