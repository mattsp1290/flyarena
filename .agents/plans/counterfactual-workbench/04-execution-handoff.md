...
Implementation has not started for this integration. Original lab code already exists and is an input, not proof that integration is complete.
...
5. Reconcile current main, review implementation, verify all gates, record evidence and commit only related changes. Stop with a clean, merge-ready branch; user did not request an actual merge or push.
...
- `bash -n scripts/deploy.sh scripts/lab.sh`; offline backend bundle content check; static production build served under `/fly/` exercises all routes and base-relative graph loading. Do not call deploy mode or remote deployment hosts.
...
## Definition of done
One app retains main's default connectome arena, adds an actual graph-based fork/silence/compare experiment with reproducible evidence, and makes the existing GPU synthetic sandbox accessible without weakening provenance or requiring a server for the static modes. All plan success criteria and merge gates have authoritative current evidence. The branch and worktree are clean and ready for review/merge; no deployment or Beans mutation has occurred.
